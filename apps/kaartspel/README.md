...
> **Werknaam.** `kaartspel` is een tijdelijke naam; definitieve branding (naam, domein, appId, kleurthema) volgt in de polish-fase.
...
- **Yousef-scoring**: ronde-punten als verschil tot de afleger, Assaf-straf bij een verkeerde call, en een bak-meter per speler (bak trekken bij >= 30, halve bak afkopen voor slokken).
...
> **Status.** Dit is een scaffold, geforkt van Kingsen. De app-schil (net/store/protocol/screens/cards/i18n) draait al; de Yousef-engine wordt in de volgende chunks gebouwd (zie [docs/plan.md](docs/plan.md)).
