...
import { cryptoDeckSource, RANKS, SUITS, type DeckSource } from '@/engine/deck'
  ...
  switch (intent.t) {
    ...
    case 'SET_RULES':
      return (
        ...
        typeof intent.rules.bluffen === 'boolean' &&
        Number.isInteger(intent.rules.busLengte)
      )
    case 'ANSWER':
      return typeof intent.choice === 'string'
    case 'GIVE_SIPS':
    case 'CALL_BLUFF':
      return typeof intent.targetPlayerId === 'string'
    case 'PLAY_CARD':
      return (
        typeof intent.card === 'object' &&
        intent.card !== null &&
        (SUITS as string[]).includes(intent.card.suit) &&
        (RANKS as number[]).includes(intent.card.rank)
      )
    case 'BUS_GUESS':
      return typeof intent.choice === 'string' && Number.isInteger(intent.position)
    case 'LEAVE':
    case 'START_GAME':
    case 'FLIP_PYRAMID':
    case 'NEXT_PHASE':
...
/**
 * Wat guests te zien krijgen: zonder de nog te trekken kaarten en zonder het
 * bluf-oordeel, anders leest een guest met devtools de bus uit of ziet hij
 * of een claim gelogen is. De eigen host-store houdt de volledige state.
 */
export function publicState(state: GameState): GameState {
  const pub = structuredClone(state)
  pub.deck = []
  pub.drawIndex = 0
  if (pub.pyramid?.openClaim) pub.pyramid.openClaim.truthful = false
  return pub
}
...
function intentToCommand(intent: Intent, playerId: string): Command | null {
  switch (intent.t) {
    ...
    case 'BUS_GUESS':
      return { t: 'BUS_GUESS', playerId, choice: intent.choice, position: intent.position }
    case 'NEXT_PHASE':
      return { t: 'NEXT_PHASE' }
    case 'END_GAME':
      return { t: 'END_GAME' }
