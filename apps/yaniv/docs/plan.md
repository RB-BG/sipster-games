...
hergebruikt; alleen `src/engine/` is echt nieuw, want Yaniv speelt heel anders dan kingsen:
beurten met hand-management + ronde-scoring + cumulatieve bak-drempel, i.p.v. één cirkel van 52
kaarten.

De spelregels staan in [`yaniv-regels.md`](./yaniv-regels.md) (bron van waarheid).

Branding vastgelegd: naam **Yaniv**, appId `games.sipster.yaniv`, domein `yaniv.sipster.games`,
zeegroen-thema (teal + turquoise + koraal).

## Chunks (elk op een eigen branch, volledig af: lint/build/test groen + merge `--no-ff`)

0. **Regels** (dit + `yaniv-regels.md`): open punten beslissen vóór de engine-chunk.
1. **Scaffold** (`feat/yaniv-scaffold`): app-skelet geforkt van kingsen. **Namespaces uniek
...
2. **Engine (puur)** (`feat/yaniv-engine`): `types`, `cardActions` (afleggen van
...
3. **Hotseat + GameScreen** (`feat/yaniv-hotseat`): `gameStore`, `useGameAdapter`,
...
4. **P2P** (`feat/yaniv-p2p`): `protocol/messages.ts`, `net/*`, netStore, lobby, QR-join.
   Pariteit met kingsen.
5. **Polish** (`feat/yaniv-polish`): geluid/haptiek, bak-/slokken-effecten (evt.
...
- Chunk 5 (polish): afgerond. Regel-uitleg herschreven naar Yousef (hand, Yousef roepen,
  bakken) in NL+EN; geluid + haptiek op afleggen/Yousef/bak; mute-knop in de game-header;
  kingsen-restanten opgeruimd (`CupMeter`, `cardActions.ts`, dode i18n-keys verwijderd).
- Feedback-ronde: Yousef nu bij 5 of lager, laatste ronde na een call (elke andere speler nog
  één beurt) via `finalTurns`, kaart-aantallen op het scorebord, hotseat review-na-trekken met
  bewuste "geef door", joker vult de kaart net als de andere. 77 vitest-tests groen.
- Huisregels: joker-wildcard, Assaf-scoring en bak-drempel instelbaar via de RulesEditor.
- Branding: naam **Yaniv**, map `apps/yaniv`, appId `games.sipster.yaniv`, domein
  `yaniv.sipster.games`, zeegroen-thema. Unieke namespaces `yaniv-`/`yaniv.*`.
  Nog device-only: kaart-flip-animaties + viewState-freeze, en de twee-device P2P-rooktest.
