# Yaniv: project-configuratie

Webbased versie van het kaart-borrelspel *Yaniv* (spelers roepen de call ook als "Yousef"; hand van 5 kaarten, laag houden, "Yousef" roepen bij handwaarde 5 of lager). P2P multiplayer, 2.5D-speelkaarten, mobile-first, meertalige UI (NL + EN). Geforkt van de architectuur van het zusterspel Kingsen; alleen het spel en de branding verschillen, de app-structuur is gelijk.

Branding: naam **Yaniv**, appId `games.sipster.yaniv`, domein `yaniv.sipster.games`, zeegroen-thema (teal + turquoise + koraal). De in-game call heet nog "Yousef" (de traditie van de tafel); dat is puur een i18n-tekst.

## Plan & voortgang

Yaniv speelt in beurten: afleggen (los/set/straat), één kaart trekken, en "Yousef" roepen bij een lage hand. Na een call krijgt elke andere speler nog één beurt, dan volgt de scoring. Punten stapelen cumulatief; vanaf 30 trek je een bak. Zie `docs/plan.md` voor de chunk-indeling en voortgang. Werk per feature op een eigen branch en rond volledig af (lint/build/test groen + commit + merge naar main met `--no-ff`) voor je aan het volgende begint.

**De spelregels in `docs/yaniv-regels.md` zijn de bron van waarheid voor de engine.** Wijk daar niet stiekem van af omdat een andere Yaniv/Yousef-variant het anders doet.
...
- **Unieke namespaces**: `PEER_PREFIX` (`yaniv-`), locale-sleutel (`yaniv.locale`), sound mute-sleutel (`yaniv.muted`), storage-sleutels (`yaniv.profile`/`yaniv.rules`). Wijzig deze nooit terug naar een gedeelde waarde, anders botsen de apps op dezelfde PeerJS-broker/origin.
